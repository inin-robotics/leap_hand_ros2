...
class DynamixelClient:
    ...
    def read_pos_vel_cur(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns the current positions and velocities and currents."""
        pos, vel, cur = self._pos_vel_cur_reader.read()
        pos = self.remove_offsets(self._pos_vel_cur_reader.motor_ids, pos)
        return pos, vel, cur
    def read_pos_vel(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns the current positions and velocities."""
        pos, vel = self._pos_vel_reader.read()
        pos = self.remove_offsets(self._pos_vel_reader.motor_ids, pos)
        return pos, vel
    def read_pos(self) -> np.ndarray:
        """Returns the current positions and velocities."""
        positions = self._pos_reader.read()
        return self.remove_offsets(self._pos_reader.motor_ids, positions)
    ...
    def write_desired_pos(self, motor_ids: Sequence[int],
        ...
        positions = (positions + np.pi) % (2 * np.pi) - np.pi
        positions = self.add_offsets(motor_ids, positions)
    ...
    def init_offsets(self, side):
        motor_ids = self._pos_reader.motor_ids
        curr_pos = self._pos_reader.read()
    ...
    def add_offsets(self, motor_ids, positions):
